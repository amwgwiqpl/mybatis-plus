...
public class LambdaUpdateWrapper<T> extends AbstractLambdaWrapper<T, LambdaUpdateWrapper<T>>
    ...
    public LambdaUpdateWrapper<T> setApply(boolean condition, SFunction<T, ?> column, String valueSql, Object... values) {
        ...
        if (StringUtils.MP_SQL_PLACE_HOLDER.matcher(valueSql).matches()) {
            valueSql = formatSqlMaybeWithParam(valueSql, null, values);
        }
        sqlSet.add(columnToString(column) + Constants.EQUALS + valueSql);
        return typedThis;
    }
